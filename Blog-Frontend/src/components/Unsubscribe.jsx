...
import { CheckCircle2, BellOff, AlertCircle, Home } from "lucide-react";
    ...
    (async () => {
      ...
      try {
        const endpoint = authorEmail
          ? `/api/subscribers/author/email/${encodeURIComponent(authorEmail)}/unsubscribe?email=${encodeURIComponent(email)}`
          : `/api/subscribers/unsubscribe?email=${encodeURIComponent(email)}`;
        ...
        if (active) {
          ...
          toast({
            ...
            description: authorEmail
              ? "You won't receive emails from this author."
              : "You've left the AIDEN mailing list.",
          });
        }
      } catch (err) {
        if (active) {
        ...
        <div className="max-w-md mx-auto text-center">
          ...
          {status === "success" && (
            <>
              ...
              <p className="text-ink-muted mb-8">
                {authorEmail
                  ? "You won't receive emails from this author anymore."
                  : "You've been removed from the AIDEN mailing list. We'll miss you."}
              </p>
          ...
          {status === "error" && (
            <>
              ...
              <p className="text-ink-muted mb-8">
                {email
                  ? "Something went wrong. Try again later, or contact us if it keeps failing."
